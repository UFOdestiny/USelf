...
#     '''
#
#     def __init__(self, device, adj_mx, n_filters, max_diffusion_step, filter_type,num_rnn_layers, cl_decay_steps, use_curriculum_learning=True, **args):
...
#
#         self.fc = nn.Linear(in_features=25, out_features=5).to(self.device)
#
...
#         go_symbol = torch.zeros(d, b, self.node_num, self.output_dim).to(self.device)
#
...
#
#         outputs = self.decoder(target, self.supports, context, teacher_forcing_ratio=teacher_forcing_ratio)
#         # print("decode")
#
#         o = outputs[1:, :, :].permute(1, 0, 2)  # outputs.permute(1, 0, 2)
#         # print(o.shape,self.output_dim)
#         o = o.reshape(b, 1, n, self.output_dim * self.output_dim)
#         # print("reshape")
#         o = self.fc(o).to(self.device)
#
#         # o = outputs[1:, :, :].permute(1, 0, 2).reshape(b, t, n, d, self.output_dim)
#         # o = outputs.reshape(b, t, n, self.output_dim)
#         # print("out")
#         return o
#
#
...
#             inputs, (seq_length, batch_size, -1))
#
#         outputs = torch.zeros(seq_length, batch_size, self.node_num * self.output_dim).to(self.device)
#         # print(outputs.shape)
#
...
class DCGRUCell(nn.Module):
    ...
    def forward(self, inputs, supports, state):
        ...
        value = torch.sigmoid(fn(inputs, supports, state, output_size, bias_start=1.0))
        ...
        if self._num_proj is not None:
            ...
            output = torch.reshape(self.project(output), shape=(batch_size, self.output_size))
...
class DCRNN(BaseModel):
    def __init__(self, device, adj_mx, n_filters, max_diffusion_step,
        ...
        # Encoder and Decoder with configurable input and output dimensions
        self.encoder = DCRNNEncoder(device=device,
                                    node_num=self.node_num,
                                    input_dim=self.input_dim,
                                    hid_dim=n_filters,
    ...
    def forward(self, source, target, iter=None):  # (b, t, n, f)
        ...
        teacher_forcing_ratio = self._compute_sampling_threshold(iter) if \
...
class DCRNNEncoder(nn.Module):
    def __init__(self, device, node_num, input_dim, hid_dim, \
        ...
        encoding_cells = [DCGRUCell(device=device, node_num=node_num, input_dim=input_dim,
                                    num_units=hid_dim, max_diffusion_step=max_diffusion_step,
                                    filter_type=filter_type)]
        for _ in range(1, num_rnn_layers):
            encoding_cells.append(DCGRUCell(device=device, node_num=node_num,
                                            input_dim=hid_dim, num_units=hid_dim,
                                            max_diffusion_step=max_diffusion_step,
                                            filter_type=filter_type))
        self.encoding_cells = nn.ModuleList(encoding_cells)
    ...
    def forward(self, inputs, supports, initial_hidden_state):
        ...
        for i_layer in range(self.num_rnn_layers):
            ...
            output_inner = []
            for t in range(seq_length):
                _, hidden_state = self.encoding_cells[i_layer](current_inputs[t, ...], supports, hidden_state)
                output_inner.append(hidden_state)
            output_hidden.append(hidden_state)
            current_inputs = torch.stack(output_inner, dim=0)
        return output_hidden, current_inputs
    ...
    def init_hidden(self, batch_size):
        return torch.stack([cell.init_hidden(batch_size) for cell in self.encoding_cells], dim=0)
...
class DCGRUDecoder(nn.Module):
    def __init__(self, device, node_num, input_dim, hid_dim, output_dim, \
        ...
        decoding_cells = [DCGRUCell(device=device, node_num=node_num, input_dim=input_dim,
                                    num_units=hid_dim, max_diffusion_step=max_diffusion_step,
                                    filter_type=filter_type)]
        for _ in range(1, num_rnn_layers - 1):
            decoding_cells.append(DCGRUCell(device=device, node_num=node_num, input_dim=hid_dim,
                                            num_units=hid_dim, max_diffusion_step=max_diffusion_step,
                                            filter_type=filter_type))

        cell_with_projection = DCGRUCell(device=device, node_num=node_num, input_dim=hid_dim,
                                         num_units=hid_dim, max_diffusion_step=max_diffusion_step,
                                         filter_type=filter_type, num_proj=output_dim)
        decoding_cells.append(cell_with_projection)
        self.decoding_cells = nn.ModuleList(decoding_cells)
    ...
    def forward(self, inputs, supports, initial_hidden_state, teacher_forcing_ratio=0.5):
        ...
        for t in range(1, seq_length):
            ...
            for i_layer in range(self.num_rnn_layers):
                hidden_state = initial_hidden_state[i_layer]
                output, hidden_state = self.decoding_cells[i_layer](current_input, supports, hidden_state)
                current_input = output
                next_input_hidden_state.append(hidden_state)
